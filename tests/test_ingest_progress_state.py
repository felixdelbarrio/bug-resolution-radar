...
def test_progress_start_clears_previous_messages() -> None:
    ...
    ingest_page._progress_finish(
        "jira", state="success", summary="run 1 ok", run_id=run_id_1
    )
...
def test_progress_ignores_stale_messages_from_previous_run() -> None:
    ...
    ingest_page._progress_finish(
        "helix", state="success", summary="run viejo", run_id=run_id_1
    )
    ...
    ingest_page._progress_finish(
        "helix", state="success", summary="run 2 ok", run_id=run_id_2
    )
